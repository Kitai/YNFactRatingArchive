    ...
    _init: function () {
        ...
        for (var el = this.elements.length-1; el >= 0; --el) {
          ...
          /*  // Function
            ...
            // Pause
              ...
              // Have to use this hack (IIFE) to make sure "elementsToShow" is interpreted right away
              (function(a, b, c) {
                  domelements[el] = $(document.createElement("pause")).attr("value", a).attr("next", b).addClass(t.cssPrefix + "pause");
                  if (currentElement.hasOwnProperty("tempMessage"))
                      domelements[el].append($("<p>").append(currentElement.tempMessage));
        ...
        for (el in domelements) {
          ...
          /*else if (ele.hasOwnProperty("func")) {
              ...
              var del = domelements[el], nRT = ele.newRT, next = del.attr("next").split(','), value = ele.pause.toString().match("^key(.*)");
